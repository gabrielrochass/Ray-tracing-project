...
class malha {
public:
    malha() : num_triangulos(0), num_vertices(0) {}

    malha(const triangulo& t) {
        add(t);
    }
    ...
    void add(const triangulo& t) {
        lista_triangulos.push_back(t);
        num_triangulos++;

        for (const auto& v : {t.vertice1, t.vertice2, t.vertice3}) {
    ...
    bool hit(const raio<double>& r, double t_min, double t_max, hit_record& rec) const {
        ...
        for (int i = 0; i < num_triangulos; i++) {
            if (lista_triangulos[i].hit(r, t_min, t_mais_proximo, temp_rec)) {
                acertou_algo = true;
                t_mais_proximo = temp_rec.t;

                rec.indice_tri = i;
                rec.normal = lista_normais[i]; // Ajuste para usar a normal correta
                rec = temp_rec; // Atualiza o registro de interseção
            }
        }
