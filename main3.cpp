...
vetor<T> raioColor(const raio<T>& raio, const vetor<T>& sphereCenter, T sphereRadius) {
    double tSphere = hitEsfera(sphereCenter, sphereRadius, raio);
    double tPlano = hitPlano({0.0, -1.25, -1.0}, {0.0, 1.0, 0.0}, raio);
    
    // Se o raio intersecta tanto a esfera quanto o plano
    if (tSphere > 0.0 && tPlano > 0.0) {
        vetor<T> p = raioAt(raio, tSphere);
        // Se o ponto de interseção estiver abaixo do plano, retorna a cor da esfera
        if (p.y < -1.25) {
            vetor<T> N = vetorUni(subtracao(p, sphereCenter));
            vetor<T> color = mult(0.5, soma(vetor<T>(1.0, 1.0, 1.0), N));
            return color;
        } else {
            // Caso contrário, retorna a cor do plano
            return vetor<T>(0.0, 1.0, 0.0);
        }
    }
    // Se o raio intersectar apenas a esfera, retorna a cor da esfera
    else if (tSphere > 0.0) {
        vetor<T> p = raioAt(raio, tSphere);
        vetor<T> N = vetorUni(subtracao(p, sphereCenter));
        vetor<T> color = mult(0.5, soma(vetor<T>(1.0, 1.0, 1.0), N));
    ...
    // Se o raio intersectar apenas o plano, retorna a cor do plano
    else if (tPlano > 0.0) {
        return vetor<T>(0.0, 1.0, 0.0);
    }

    // Se o raio não intersectar nem a esfera nem o plano, retorna a cor de fundo
